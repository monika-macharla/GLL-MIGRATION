...
class HSCreditSummaryMigrator(BaseMigrator):

    SOURCE_TABLE = "hs_transcript"
    DESTINATION_TABLE = "import_credit_summary"
    ...
    def migrate(self) -> int:
        ...
        logger.info(
            "Starting HS Credit Summary Import Migration..."
        )
        ...
        logger.info(
            f"Source hs_transcript count: {source_count}"
        )
        logger.info(
            f"Destination import_credit_summary current count: "
            f"{destination_count}"
        )
        ...
        while True:
            ...
            for row in rows:
                ...
                if not student_number:
                    ...
                    student_number = f"HS-TRANSCRIPT-{source_id}"
                ...
                mapped_row = {
                    ...
                    "state_credits": self._round_decimal(
                        row_dict.get(
                            source_table.c.state_credits
                        )
                    ),
                    "local_credits": self._round_decimal(
                        row_dict.get(
                            source_table.c.local_credits
                        )
                    ),
                    "institution_id": (
            ...
            if insert_data:
                ...
                logger.info(
                    "Inserting import_credit_summary chunk "
        ...
        logger.info(
            "HS Credit Summary Import Summary: "
    ...
    def _build_institution_uuid_lookup(
        ...
        logger.info(
            "Loaded hs_credit_summary institution UUID lookup: "
            f"mapped={len(lookup)}, "
    ...
    def _stable_uuid(
        ...
        return str(
            uuid.uuid5(
                uuid.NAMESPACE_URL,
                f"gll:import-credit-summary:{source_id}"
            )
        )
