...
class HSAwardingCreditMigrator(BaseMigrator):

    SOURCE_TABLE = "hs_awarding_credit"
    DESTINATION_TABLE = "import_schools_awarding_credits"
    ...
    def migrate(self) -> int:
        ...
        logger.info(
            "Starting HS Awarding Credit Import Migration..."
        )
        ...
        logger.info(
            f"Source hs_awarding_credit count: {source_count}"
        )
        logger.info(
            "Destination import_schools_awarding_credits current count: "
            f"{destination_count}"
        )
        ...
        while True:
            ...
            for row in rows:
                ...
                mapped_row = {
                    ...
                    "school_number": self._truncate(
                        row_dict.get(
                            source_table.c.school_number
                        ),
                        50
                    ),
                    "school_name": self._truncate(
                        row_dict.get(
                            source_table.c.school_name
                        ),
                        255
                    ),
                    "institution_id": (
            ...
            if insert_data:
                ...
                logger.info(
                    "Inserting import_schools_awarding_credits chunk "
        ...
        logger.info(
            "HS Awarding Credit Import Summary: "
    ...
    def _stable_uuid(
        ...
        return str(
            uuid.uuid5(
                uuid.NAMESPACE_URL,
                f"gll:import-schools-awarding-credits:{source_id}"
            )
        )
