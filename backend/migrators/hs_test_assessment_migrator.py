...
class HSTestAssessmentMigrator(BaseMigrator):

    SOURCE_TABLE = "hs_test_assessment"
    DESTINATION_TABLE = "import_student_test_assessments"
    ...
    def migrate(self) -> int:
        ...
        logger.info(
            "Starting HS Test Assessment Import Migration..."
        )
        ...
        logger.info(
            f"Source hs_test_assessment count: {source_count}"
        )
        logger.info(
            "Destination import_student_test_assessments current count: "
            f"{destination_count}"
        )
        ...
        while True:
            ...
            for row in rows:
                ...
                mapped_row = {
                    ...
                    "date_tested": self._truncate(
                        row_dict.get(
                            source_table.c.date_tested
                        ),
                        10
                    ),
                    "course_code": self._truncate(
                        row_dict.get(
                            source_table.c.test_code
                        ),
                        50
                    ),
                    "test_name": self._truncate(
                        row_dict.get(
                            source_table.c.test_name
                        ),
                        100
                    ),
                    "test_score_name": self._truncate(
                        row_dict.get(
                            source_table.c.test_score_name
                        ),
                        100
                    ),
                    "score": self._truncate(
                        row_dict.get(
                            source_table.c.score
                        ),
                        50
                    ),
                    "level": self._truncate(
                        row_dict.get(
                            source_table.c.level
                        ),
                        50
                    ),
                    "performance_level": self._truncate(
                        row_dict.get(
                            source_table.c.performance_level
                        ),
                        50
                    ),
                    "institution_id": (
            ...
            if insert_data:
                ...
                logger.info(
                    "Inserting import_student_test_assessments chunk "
        ...
        logger.info(
            "HS Test Assessment Import Summary: "
    ...
    def _build_institution_uuid_lookup(
        ...
        logger.info(
            "Loaded hs_test_assessment institution UUID lookup: "
            f"mapped={len(lookup)}, "
    ...
    def _stable_uuid(
        ...
        return str(
            uuid.uuid5(
                uuid.NAMESPACE_URL,
                f"gll:import-student-test-assessment:{source_id}"
            )
        )
